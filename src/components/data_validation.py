...
class DataValidation:
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        ...
        try:
            ...
            #validation col type fort train /test df
            status = self.is_column_exist(df = train_df)
            if not status:
                validation_error_msg += f"Columns are msising in training dataframe."
            else:
                logging.info(f"All cateorical/int columns present in training datafeame:{status}")
            ...
            # save validataion status and message to a JSON filer 
